...
val interceptor = Interceptor(invoke = fun(options: NavigateToOptions) {
    console.log("拦截 navigateTo 接口传入参数为：", options, " at App.uvue:5");
    val token = uni_getStorageSync("token");
}
, success = fun(res: NavigateBackSuccess) {
    console.log("拦截 navigateTo 接口 success 返回参数为：", res, " at App.uvue:17");
}
, fail = fun(err: NavigateToFail) {
    console.log("拦截 navigateTo 接口 fail 返回参数为：", err, " at App.uvue:20");
}
, complete = fun(res: NavigateToComplete) {
    console.log("拦截 navigateTo 接口 complete 返回参数为：", res, " at App.uvue:23");
}
);
